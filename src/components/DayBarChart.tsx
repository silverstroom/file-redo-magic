...
const REMAINING_COLOR = 'hsl(0, 0%, 92%)';
...
export function DayBarChart({ distribution }: DayBarChartProps) {
  ...
  return (
    <div className="soft-card p-5">
      ...
      {editing && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5 p-3 rounded-xl bg-muted/50">
          {distribution.map((d) => (
            <div key={d.date} className="flex items-center gap-2">
              <label className="text-xs font-medium text-muted-foreground whitespace-nowrap min-w-[50px]">
                {d.day}
              </label>
              <Input
                type="number"
                ...
                className="h-8 text-sm font-mono rounded-lg"
              />
            </div>
          ))}
        </div>
        ...
        {distribution.map((d, i) => {
          ...
          return (
            <div key={d.date} className="flex flex-col items-center">
              ...
              <div className="h-[100px] w-[100px] sm:h-[140px] sm:w-[140px]">
