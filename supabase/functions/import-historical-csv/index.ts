...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!csvText) return new Response(JSON.stringify({ success: false, error: 'No CSV data' }),
    ...
    for (let i = 0; i < rows.length; i += 500) {
      ...
      const { error } = await sb.from('historical_daily_presenze').upsert(batch, { onConflict: 'edition_key,sale_date' });
      if (error) throw new Error(`Batch insert error: ${error.message}`);
    }

    const editions = [...new Set(rows.map(r => r.edition_key))].sort();
    return new Response(JSON.stringify({ success: true, processed, skipped, uniqueDays: aggregated.size, editions }),
